...
MongoClient.connect(mongoURL, async (err, database) => {
    ...
    listings = await db.collection('Listings', {
        validator: {
            $jsonSchema: {
                bsonType: "object",
                required: ["name", "host", "description", "price", "rating", "location", "size", "time", "booked", "attributes", "image"],
                properties: {
                    name: {
                        bsonType: "string"
                    },
    ...
    users = await db.collection('Users', {
        validator: {
            $jsonSchema: {
                bsonType: "object",
                required: ["username", "password", "firstName", "lastName", "host", "bookings", "profileImage"],
                properties: {
                    username: {
                        bsonType: "string"
                    },
...
app.post('/book_listing', auth, async (req, res) => [
    await listings.update(
        ...
        {$push: {booked: {startTime: req.body.startTime, endTime: req.body.endTime}}}
    )
])
...
app.post('/add_user', async (req, res) => {
    const userAlreadyExists = await users.findOne({username: req.body.username});
    ...
    users.insertOne(newUser, async (err, result) => {
        ...
        const token = await jwt.sign({_id: result.insertedId}, process.env.TOKEN_SECRET);
        return res.status(200).cookie("token", token);
    })
})